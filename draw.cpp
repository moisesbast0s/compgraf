...
// --- FUNÇÃO 1: A Geometria do Losango ---
static void desenhaLosango(float altura)
{
    float h = altura / 2.0f;
    float s = altura / 3.0f;

    // Cores (Comentadas para deixar o Shader controlar a cor)
...
// --- FUNÇÃO 2: Torres com Textura + Losangos com Shader ---
void desenhaTorresELosangos()
{
    ...
    for (int i = 0; i < NUM_TORRES; i++)
    {
        ...
        // --- PARTE A: TORRE (Com Textura) ---
        glPushMatrix();

        // Garante que a textura está ligada para a torre
        ...
        float dx = x - camX;
        float dz = z - camZ;
        float distanciaReal = sqrt(dx * dx + dz * dz);

        
        GLint locDist = glGetUniformLocation(progLosango, "uDistancia");
        glUniform1f(locDist, distanciaReal);
